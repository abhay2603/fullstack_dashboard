...
# # ---- FETCH DATA FROM API ----
# @st.cache_data
# def get_data_from_api(source='excel', gender='Male', branch=None, city=None):
#     url = SALES_API_URL
#     params = {
#         'source': source
#      }
...
# # Debugging outputs
# # st.write("DataFrame loaded from API:")
# # st.write(df)

# # Ensure columns exist in DataFrame
# # if not df.empty:
# #     st.write("Columns in DataFrame:", df.columns.tolist())
# #     st.write("Unique values for 'City':", df["City"].unique())
# #     st.write("Unique values for 'Branch':", df["Branch"].unique())
# #     st.write("Unique values for 'Gender':", df["Gender"].unique())
...
#     df_selection = get_data_from_api(
#     source=source,
#       # Assuming 'branch' corresponds to 'city' filter
#     city=city[0] if city else None,
#     gender=gender[0] if gender else None,
#     branch=customer_type[0] if customer_type else None
# )

# # Debugging output for filtered DataFrame
...
# sales_by_hour = df_selection.groupby(by=["Gender"])[["Total"]].sum()
